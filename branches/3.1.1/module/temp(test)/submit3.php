...
function oj_to_test($src, $language, $pid, $protype, $contest_id, $exam_id, $uid, $user_id)
{
    $curl=curl_init();

    $url="http://120.76.240.198/gdinoj/server/?action=problem.submit2";
    $body="source='$src'&language=$language&pid=$pid&protype=$protype&contest_id=$contest_id&exam_id=$exam_id&uid=$uid&user_id=$user_id";
    curl_setopt ( $curl, CURLOPT_URL, $url );
    curl_setopt ( $curl, CURLOPT_POST, 1 );
    curl_setopt ( $curl, CURLOPT_POSTFIELDS, $body);
    $contest=curl_exec ( $curl );
    echo $contest;
    curl_close($curl);
}
    ...
    switch ($protype) {
        ...
        case '3'://代码题
            ...
            if(($res['last_submit']!=null && time()-strtotime($res['last_submit'])>30) || $res['last_submit']==null)
            {
                ...
                oj_to_test($source, $language, $pid, $protype, $contest_id, $exam_id, $uid, $user_id);
                OutPut(true);
